...
class JBEngine(PwRStudioEngine):
    ...
    def __init__(self, project: str, progress, credentials: dict = None, session_id: str = None, istest:bool=False):
        if not istest:
    ...
    async def _process_utterance(self, text, **kwargs):
        ...
        def status_update_callback_wrapper(event, data):
            if event == "plan_generated":
                status_update_callback(f"Plan successfully generated! 🎉 ({len(data)} steps)\n\n")
                status_update_callback(f"Applying the plan...")
            elif event == "step_update":
                step = data["step"]
                step_number = data["step_number"]
                status_update_callback(f"Applying step {step_number}({step['type']}) of the plan for task {step['task_id']}...")
            elif event == "flow_update_completed":
                status_update_callback(f"All steps in the plan have been applied successfully! 🎉")
        ...
        feedback = generate_feedback(chat_history_strings + [text,], nlr, errors)
        ...
        program_state = {
            "errors": len(""), "warnings": 0, "optimizations": 0, "botExperience": "80%"
        }
    ...
    async def _process_import(self, **kwargs):
        ...
        program_state = {
            "errors": 0, "warnings": 0, "optimizations": 0, "botExperience": "80%"
        }
        # await self._progress(
        #     Response(
        #         # type="dsl_state",
