...
def make_message_for_variable_list(var_type, var_list):
    ...
    msg = (
        cvar_config
        + "\xa1"
        + f"Enter the values for the following {var_type}:\n\n"
        + "\n\n".join(all_cvar_names)
    )
    return msg
...
def convert_dsl(dsl: str) -> str:
    ...
    if len(new_dsl["config_vars"]) > 0:
        ...
        first_config_task["message"] = (
            "Welcome, to start the program, please provide the following key/secret values."
        )
        ...
        for i, cfg in enumerate(new_dsl["config_vars"]):
            ...
            if i == 0:
                dsl_msg = make_message_for_variable_list(
                    "configuration variables",
                    [x["name"] for x in new_dsl["config_vars"]],
                )
    ...
    for i in range(len(new_dsl["dsl"]) - 1, -1, -1):
        ...
        if task["task_type"] == "plugin":
            ...
            api_result_branch["read_variables"] = [
                name + "_code",
            ]
            ...
            for j, v in enumerate(output_vars):
                ...
                if j == 0:
                    ...
                    if len(output_vars) > 2:
                        ...
                        pg_msg = make_message_for_variable_list(
                            "plugin outputs", output_vars[1:]
                        )
    ...
    for step in new_dsl["dsl"]:
        if step["task_type"] == "input":
            ...
            print(step)
            print(step.get("options"))
            if (
                "options" in step
                and step["options"] is not None
                and len(step["options"]) > 0
            ):
